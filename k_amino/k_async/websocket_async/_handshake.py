...
async def handshake(sock, url, hostname, port, resource, **options):
    headers, key = _get_handshake_headers(resource, url, hostname, port, options)

    header_str = "\r\n".join(headers)
    await send(sock, header_str)
    dump("request header", header_str)

    status, resp = await _get_resp_headers(sock)
...
async def _get_resp_headers(sock, success_statuses=SUCCESS_STATUSES):
